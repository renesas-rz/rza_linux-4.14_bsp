...
function usage {
  ...
  echo -e "    ./build.sh axfs                    : Builds an AXFS images"
  echo -e ""
...
FILE2=output/linux-4.14/arch/arm/boot/dts/r7s72100-$BOARD.dtb
FILE3=output/linux-4.14/arch/arm/boot/uImage
FILE4=output/linux-4.14/arch/arm/boot/xipImage
...
if [ "$1" == "kernel" ] || [ "$1" == "k" ] ; then
  ...
  if [ ! -e linux-4.14 ] ;then
    ...
    git clone https://github.com/renesas-rz/rza_linux-4.14.git linux-4.14
  fi
...
if [ "$1" == "buildroot" ]  || [ "$1" == "b" ] ; then
  ...
  if [ "$BR_VERSION" == "2017.02" ] ; then
    ...
    # Created by doing this in the Buildroot mainline repoistory:
